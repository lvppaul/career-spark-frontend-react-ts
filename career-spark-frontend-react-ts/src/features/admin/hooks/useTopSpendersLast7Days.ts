...
/**
 * Hook to fetch top spenders for last 7 days in admin dashboard
 */
export function useTopSpendersLast7Days(initialParams?: GetTopSpendersParams) {
  ...
  const fetch = React.useCallback(
    async (fetchParams?: GetTopSpendersParams) => {
      ...
      try {
        ...
        const resp =
          await adminOrderService.getTopSpendersLast7Days(queryParams);
        if (resp.success) {
          setData(resp.data || []);
        } else {
        ...
        console.error('Failed to fetch top spenders last 7 days', err);
        setError(err as Error);
        message.error('Không thể tải dữ liệu top người chi tiêu 7 ngày');
      } finally {
        setIsLoading(false);
      }
    },
...
export default useTopSpendersLast7Days;
